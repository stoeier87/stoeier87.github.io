...
Canonical contract for this repo. Two stubs at repo root point here: `CLAUDE.md`, which `@`-imports
this file so it auto-loads (Claude Code discovers context by filename at root, never by directory
name), and `AGENTS.md`, which is where other tools look.
