...
<svg class="egg-console" viewBox="0 0 96 100" aria-hidden="true">
  ...
  <path class="cable" fill="none" d="M 42 63 C 38 76, 52 84, 46 100" />
    ...
    </div>`,t}function a(){let n=document.createElement(`style`);n.textContent=t,document.head.appendChild(n);let r=i();document.body.appendChild(r);let a=r.querySelector(`.egg-scene`),o=r.querySelector(`.egg-bubble`),s=r.querySelector(`.egg-button`);s.tabIndex=-1;let c=!1,l=0;function u(){let e=document.documentElement.scrollHeight-window.innerHeight-window.scrollY<=200;e!==c&&(c=e,r.classList.toggle(`egg-in`,c),s.tabIndex=c?0:-1,c||g(!0),c&&!f&&l===0&&(f=!0,setTimeout(()=>{c&&l===0&&(h(d),m=setTimeout(()=>g(!1),4500))},500)))}window.addEventListener(`scroll`,u,{passive:!0}),window.addEventListener(`resize`,u,{passive:!0}),u();let d=`Whatever you do, do not click the button.`,f=!1,p=null,m=null;function h(e){clearInterval(p),clearTimeout(m),o.textContent=``,o.classList.add(`show`);let t=0;p=setInterval(()=>{o.textContent=e.slice(0,++t),t>=e.length&&clearInterval(p)},40)}function g(e){clearInterval(p),clearTimeout(m),o.classList.remove(`show`),e&&(o.textContent=``)}a.addEventListener(`mouseenter`,()=>{c&&l===0&&h(d)}),a.addEventListener(`mouseleave`,()=>{l===0&&(m=setTimeout(()=>g(!1),600))}),s.addEventListener(`focus`,()=>{c&&l===0&&h(d)});let _=null,v=null;function y(){clearTimeout(m),m=setTimeout(()=>g(!1),2500)}s.addEventListener(`click`,()=>{!c||l>=4||(l+=1,l===1?(h(`Don't.`),y(),r.classList.add(`egg-recoil`,`egg-eyes-bright`),clearTimeout(_),clearTimeout(v),_=setTimeout(()=>r.classList.remove(`egg-recoil`),500),v=setTimeout(()=>r.classList.remove(`egg-eyes-bright`),900)):l===2?(h(`I said don't.`),y(),r.classList.remove(`egg-recoil`,`egg-eyes-bright`),r.classList.add(`egg-press2plus`)):l===3?(h(`Do you know how long that took to build.`),y(),r.classList.add(`egg-press3`)):(r.classList.remove(`egg-press3`),r.classList.add(`egg-press4`),h(`Fine.`),clearTimeout(m),setTimeout(C,400)))});function b(e){let t=document.createElement(`div`);return t.className=`egg-fx `+e,document.body.appendChild(t),t}function x(){let e=`http://www.w3.org/2000/svg`,t=document.createElementNS(e,`svg`);t.setAttribute(`class`,`egg-fx egg-cracks`);let n=window.innerWidth/2,r=window.innerHeight/2;for(let i=0;i<9;i++){let a=i/9*Math.PI*2+Math.random()*.5,o=n,s=r,c=`M ${o.toFixed(0)} ${s.toFixed(0)}`,l=5+Math.floor(Math.random()*3);for(let e=0;e<l;e++){a+=(Math.random()-.5)*.9;let t=40+Math.random()*120;if(o+=Math.cos(a)*t,s+=Math.sin(a)*t,c+=` L ${o.toFixed(0)} ${s.toFixed(0)}`,e===2&&Math.random()<.7){let e=a+(Math.random()<.5?1:-1)*(.6+Math.random()*.5),t=o+Math.cos(e)*(30+Math.random()*60),n=s+Math.sin(e)*(30+Math.random()*60);c+=` M ${o.toFixed(0)} ${s.toFixed(0)} L ${t.toFixed(0)} ${n.toFixed(0)} M ${o.toFixed(0)} ${s.toFixed(0)}`}}let u=document.createElementNS(e,`path`);u.setAttribute(`d`,c),u.setAttribute(`class`,`egg-crack`),u.setAttribute(`pathLength`,`1`),u.style.strokeDasharray=`1`,u.style.strokeDashoffset=`1`,u.style.transition=`stroke-dashoffset 0.7s ease-out ${(i*.08).toFixed(2)}s`,t.appendChild(u)}return document.body.appendChild(t),requestAnimationFrame(()=>requestAnimationFrame(()=>{t.querySelectorAll(`path`).forEach(e=>e.style.strokeDashoffset=`0`)})),t}function S(e,t,n,r){let i=e.getBoundingClientRect(),a=window.innerWidth/2-(i.left+i.width/2),o=window.innerHeight/2-(i.top+i.height/2);e.style.transition=`transform ${n}s cubic-bezier(0.55, -0.15, 0.75, 0.5) ${r}s, opacity 0.25s ease ${(r+n*.8).toFixed(2)}s`,e.style.transform=`translate(${a.toFixed(0)}px, ${o.toFixed(0)}px) ${t} scale(0.04)`,e.style.opacity=`0`}function C(){if(g(!0),document.body.style.overflow=`hidden`,e){let e=b(`egg-blackout`);e.style.transition=`opacity 0.8s ease`,requestAnimationFrame(()=>e.style.opacity=`1`),setTimeout(()=>w(e),900);return}let t=document.documentElement,n=[document.getElementById(`starfield`),document.querySelector(`.dotgrid`),document.querySelector(`.topbar`),document.querySelector(`main`),document.querySelector(`footer`),a].filter(Boolean);n.forEach(e=>e.classList.add(`egg-shaken`));let i=9,o=0,s=0,c=1,l=!1,u=setInterval(()=>{l&&(i=Math.min(15,i+.4),s=Math.min(4,s+.27),c=Math.min(1.08,c+.0054)),t.style.setProperty(`--egg-sx`,((Math.random()*2-1)*(i+o)).toFixed(1)+`px`),t.style.setProperty(`--egg-sy`,((Math.random()*2-1)*i).toFixed(1)+`px`),t.style.setProperty(`--egg-rot`,s.toFixed(2)+`deg`),t.style.setProperty(`--egg-scl`,c.toFixed(4))},80),d=(e,t)=>setTimeout(t,e),f=b(`egg-border-flicker`);f.classList.add(`run`),d(600,()=>f.remove());let p=document.getElementById(`stageName`);d(400,()=>p&&p.classList.add(`egg-rgb`));for(let e=0;e<5;e++)d(400+Math.random()*900,()=>{let e=document.createElement(`div`);e.className=`egg-band`,e.style.top=Math.random()*90+`vh`,e.style.height=18+Math.random()*26+`px`;let t=(20+Math.random()*40)*(Math.random()<.5?-1:1);e.style.transform=`translateX(${t}px)`,document.body.appendChild(e),o=30,setTimeout(()=>{o=0,e.remove()},100)});d(1400,()=>p&&p.classList.remove(`egg-rgb`));let m=null,_=null;d(800,()=>{m=x(),_=b(`egg-scanlines`),requestAnimationFrame(()=>_.classList.add(`show`))}),d(1200,()=>{document.querySelectorAll(`#stageName .ltr`).forEach(e=>{S(e,`rotate(${((20+Math.random()*100)*(Math.random()<.5?-1:1)).toFixed(0)}deg)`,.9,Math.random()*.4)}),document.querySelectorAll(`.topbar .pill, #contact .pill`).forEach(e=>{let t=e.getBoundingClientRect();S(e,`rotate(${(Math.atan2(window.innerHeight/2-(t.top+t.height/2),window.innerWidth/2-(t.left+t.width/2))*180/Math.PI).toFixed(0)}deg) scaleX(1.7)`,1,.15+Math.random()*.35)})});let v=b(`egg-flash`);d(2600,()=>{v.style.transition=`opacity 0.12s ease-in`,v.style.opacity=`1`,setTimeout(()=>{v.style.transition=`opacity 0.25s ease-out`,v.style.opacity=`0`},130),document.getElementById(`starfield`).classList.add(`egg-burnout`),document.querySelector(`.dotgrid`)?.classList.add(`egg-burnout`),l=!0}),d(3550,()=>h(`Told you.`)),d(3600,()=>{let e=r.querySelector(`.egg-alien-float`),t=r.querySelector(`.egg-console-wrap`);S(e,`rotate(720deg)`,.55,0),S(t,`rotate(200deg)`,.5,.12)});let y=b(`egg-blackout`);d(4200,()=>{g(!0),v.style.transition=`opacity 0.1s ease-in`,v.style.opacity=`1`,setTimeout(()=>{y.style.transition=`opacity 0.4s ease`,y.style.opacity=`1`,v.style.transition=`opacity 0.4s ease`,v.style.opacity=`0`},250)}),d(4700,()=>{clearInterval(u),n.forEach(e=>e.classList.remove(`egg-shaken`)),[`--egg-sx`,`--egg-sy`,`--egg-rot`,`--egg-scl`].forEach(e=>t.style.removeProperty(e)),m?.remove(),_?.remove(),f.remove(),v.remove()}),d(5e3,()=>w(y))}function w(){let e=document.createElement(`div`);e.className=`egg-dead`,e.innerHTML=`
