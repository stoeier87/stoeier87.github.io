...
`;function l(){let e=document.createElement(`div`);e.className=`egg-layer`+(n?``:` egg-idle`),e.innerHTML=`
    ...
    </div>`;let{svg:t,crackEls:r}=s();return e.querySelector(`.egg-station-btn`).appendChild(t),{layer:e,crackEls:r}}function u(){let e=document.createElement(`style`);e.textContent=c,document.head.appendChild(e);let{layer:t,crackEls:r}=l();document.body.appendChild(t);let a=t.querySelector(`.egg-scene`),o=t.querySelector(`.egg-station-btn`),s=t.querySelector(`.egg-station-wrap`),u=t.querySelector(`.egg-station-entrance`);o.tabIndex=-1;function d(){let e=document.getElementById(`stageName`);if(!e)return;let n=e.getBoundingClientRect(),r=a.offsetHeight||120,i=n.top+n.height/2-r/2;i=Math.max(8,Math.min(i,window.innerHeight-r-8)),t.style.top=i.toFixed(0)+`px`,t.style.bottom=`auto`}let f=!1,p=0,m=!1;function h(){let e=document.documentElement.scrollHeight-window.innerHeight-window.scrollY<=200;e&&f&&d(),e!==f&&(e&&d(),f=e,t.classList.toggle(`egg-in`,f),o.tabIndex=f?0:-1)}window.addEventListener(`scroll`,h,{passive:!0}),window.addEventListener(`resize`,h,{passive:!0}),h();let g=0,_=null;window.addEventListener(`pointermove`,e=>{_={x:e.clientX,y:e.clientY}},{passive:!0});function v(e){if(m||p>=5)return;let t=performance.now?performance.now():Date.now();t-g<250||(g=t,y(e),setTimeout(T,150))}function y(e){let t=o.getBoundingClientRect(),n=t.left+t.width/2,r=t.top+t.height/2,i=n-e.x,a=r-e.y,s=Math.atan2(a,i)*180/Math.PI,c=document.createElement(`div`);c.className=`egg-bolt`,c.style.transform=`translate(${e.x}px, ${e.y}px) rotate(${s}deg)`,document.body.appendChild(c),requestAnimationFrame(()=>{c.style.transition=`transform 0.15s linear`,c.style.transform=`translate(${(e.x+i).toFixed(0)}px, ${(e.y+a).toFixed(0)}px) rotate(${s}deg)`}),setTimeout(()=>{c.remove();let e=document.createElement(`div`);e.className=`egg-ring`,e.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px)`,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add(`run`)),setTimeout(()=>e.remove(),320)},150)}function b(){let e=o.getBoundingClientRect(),t=e.left+e.width/2,n=e.top+e.height/2,r={top:n,bottom:window.innerHeight-n,left:t,right:window.innerWidth-t},i=Math.min(r.top,r.bottom,r.left,r.right);return i===r.top?{x:t,y:0}:i===r.bottom?{x:t,y:window.innerHeight}:i===r.left?{x:0,y:n}:{x:window.innerWidth,y:n}}o.addEventListener(`click`,()=>{let e=o.getBoundingClientRect();v(_&&Math.hypot(_.x-(e.left+e.width/2),_.y-(e.top+e.height/2))>30?_:b())});function x(){let e=o.getBoundingClientRect(),t=e.left+e.width*.42,n=e.top+e.height*.4,r=_??b(),i=r.x-t,a=r.y-n,s=Math.hypot(i,a)||1,c=Math.atan2(a,i)*180/Math.PI,l=document.createElement(`div`);l.className=`egg-bolt egg-bolt-return`,l.style.transform=`translate(${t.toFixed(0)}px, ${n.toFixed(0)}px) rotate(${c}deg)`,document.body.appendChild(l),requestAnimationFrame(()=>{l.style.transition=`transform 0.22s linear, opacity 0.22s ease-in`,l.style.transform=`translate(${(t+i/s*(s+90)).toFixed(0)}px, ${(n+a/s*(s+90)).toFixed(0)}px) rotate(${c}deg)`,l.style.opacity=`0`}),setTimeout(()=>l.remove(),260)}let S=null,C=null;function w(e){r.forEach(t=>{Number(t.dataset.hit)===e&&t.classList.add(`show`)})}function T(){if(!(m||p>=5)){if(p+=1,t.classList.add(`egg-hit`+p),n||x(),w(p),!n){if(p===1&&(t.classList.add(`egg-recoil-once`),clearTimeout(S),S=setTimeout(()=>t.classList.remove(`egg-recoil-once`),500)),p===2){let e=document.createElement(`div`);e.className=`egg-spark-run`,e.style.position=`absolute`,e.style.left=`38%`,e.style.top=`30%`,e.innerHTML=`<span class="egg-spark egg-spark-1"></span><span class="egg-spark egg-spark-2"></span>`,s.appendChild(e),clearTimeout(C),C=setTimeout(()=>e.remove(),900)}if(p===4){let e=t.querySelectorAll(`.egg-plate`);e[1]&&e[1].classList.add(`egg-plate-detach`),e[4]&&e[4].classList.add(`egg-plate-detach`)}}p===5&&setTimeout(j,400)}}function E(e){let t=document.createElement(`div`);return t.className=`egg-fx `+e,document.body.appendChild(t),t}function D(){let e=i(`svg`,{class:`egg-fx egg-cracks-fx`}),t=window.innerWidth/2,n=window.innerHeight/2;for(let r=0;r<9;r++){let a=r/9*Math.PI*2+Math.random()*.5,o=t,s=n,c=`M ${o.toFixed(0)} ${s.toFixed(0)}`,l=5+Math.floor(Math.random()*3);for(let e=0;e<l;e++){a+=(Math.random()-.5)*.9;let t=40+Math.random()*120;if(o+=Math.cos(a)*t,s+=Math.sin(a)*t,c+=` L ${o.toFixed(0)} ${s.toFixed(0)}`,e===2&&Math.random()<.7){let e=a+(Math.random()<.5?1:-1)*(.6+Math.random()*.5),t=o+Math.cos(e)*(30+Math.random()*60),n=s+Math.sin(e)*(30+Math.random()*60);c+=` M ${o.toFixed(0)} ${s.toFixed(0)} L ${t.toFixed(0)} ${n.toFixed(0)} M ${o.toFixed(0)} ${s.toFixed(0)}`}}let u=i(`path`,{d:c,class:`egg-crack-fx`,pathLength:`1`});u.style.strokeDasharray=`1`,u.style.strokeDashoffset=`1`,u.style.transition=`stroke-dashoffset 0.7s ease-out ${(r*.08).toFixed(2)}s`,e.appendChild(u)}return document.body.appendChild(e),requestAnimationFrame(()=>requestAnimationFrame(()=>{e.querySelectorAll(`path`).forEach(e=>e.style.strokeDashoffset=`0`)})),e}function O(e,t,n,r){let i=e.getBoundingClientRect(),a=window.innerWidth/2-(i.left+i.width/2),o=window.innerHeight/2-(i.top+i.height/2);e.style.transition=`transform ${n}s cubic-bezier(0.55, -0.15, 0.75, 0.5) ${r}s, opacity 0.25s ease ${(r+n*.8).toFixed(2)}s`,e.style.transform=`translate(${a.toFixed(0)}px, ${o.toFixed(0)}px) ${t} scale(0.04)`,e.style.opacity=`0`}function k(e){let t=o.getBoundingClientRect(),n=t.left+t.width/2,r=t.top+t.height/2;u.style.transformOrigin=`center center`,u.style.transition=`transform 0.55s cubic-bezier(0.7, 0, 0.85, 0.2), filter 0.5s ease-in, opacity 0.15s ease 0.5s`,u.style.filter=`brightness(3.4)`,u.style.transform=`scale(0.04)`,u.style.opacity=`0`,setTimeout(()=>e({x:n,y:r}),560)}function A(e,t){let n=e?.x??window.innerWidth/2,r=e?.y??window.innerHeight/2,i=Math.max(Math.hypot(n,r),Math.hypot(window.innerWidth-n,r),Math.hypot(n,window.innerHeight-r),Math.hypot(window.innerWidth-n,window.innerHeight-r)),a=document.createElement(`div`);a.className=`egg-singularity`,a.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px) scale(0)`,document.body.appendChild(a),requestAnimationFrame(()=>{a.style.transition=`transform 0.18s ease-out, opacity 0.12s ease-out`,a.style.opacity=`1`,a.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px) scale(1.1)`}),setTimeout(()=>{a.style.transition=`transform 0.1s ease-in, opacity 0.1s ease-in`,a.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px) scale(0)`,a.style.opacity=`0`;let e=E(`egg-flash`);e.style.transition=`opacity 0.06s linear`,requestAnimationFrame(()=>e.style.opacity=`0.92`),setTimeout(()=>{e.style.transition=`opacity 0.45s ease-out`,e.style.opacity=`0`},80),setTimeout(()=>e.remove(),600);let t=document.createElement(`div`);t.className=`egg-blast`,t.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px) scale(0.3)`,document.body.appendChild(t),requestAnimationFrame(()=>{t.style.transition=`transform 0.7s cubic-bezier(0.1, 0.8, 0.3, 1), opacity 0.7s ease-in`,t.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px) scale(${(i*1.15/20).toFixed(1)})`,t.style.opacity=`0`}),setTimeout(()=>t.remove(),750),[0,90,190].forEach((e,t)=>{setTimeout(()=>{let e=document.createElement(`div`);e.className=`egg-shockwave`,e.style.setProperty(`--sw-scale`,i/10*(1+t*.35)),e.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px) scale(0.2)`,document.body.appendChild(e),requestAnimationFrame(()=>e.classList.add(`run`)),setTimeout(()=>e.remove(),900)},e)});for(let e=0;e<40;e++){let t=document.createElement(`div`);t.className=`egg-ember`,t.style.color=e%5==0?`#8fc6ff`:e%3==0?`#ffd9a0`:`#ffffff`,t.style.transform=`translate(${n.toFixed(0)}px, ${r.toFixed(0)}px)`,document.body.appendChild(t);let a=Math.random()*Math.PI*2,o=i*(.5+Math.random()*.75),s=.5+Math.random()*.45;requestAnimationFrame(()=>{t.style.transition=`transform ${s}s cubic-bezier(0.1, 0.7, 0.3, 1), opacity ${s}s ease-in`,t.style.transform=`translate(${(n+Math.cos(a)*o).toFixed(0)}px, ${(r+Math.sin(a)*o).toFixed(0)}px) scale(${(.4+Math.random()*1.6).toFixed(2)})`,t.style.opacity=`0`}),setTimeout(()=>t.remove(),1100)}},330),setTimeout(()=>{a.remove(),t()},1050)}function j(){m=!0,document.body.style.overflow=`hidden`,document.documentElement.style.overflow=`hidden`,o.style.pointerEvents=`none`;let e=t.querySelector(`.egg-wander`);if(e&&(e.style.animationPlayState=`paused`),n){let e=E(`egg-blackout`);e.style.transition=`opacity 0.8s ease`,requestAnimationFrame(()=>e.style.opacity=`1`),setTimeout(M,900);return}let r=document.documentElement,i=[document.getElementById(`sky`),document.querySelector(`.dotgrid`),document.querySelector(`.topbar`),document.querySelector(`main`),document.querySelector(`footer`),a].filter(Boolean);i.forEach(e=>e.classList.add(`egg-shaken`));let s=9,c=0,l=0,u=1,d=!1,f=setInterval(()=>{d&&(s=Math.min(15,s+.4),l=Math.min(4,l+.27),u=Math.min(1.08,u+.0054)),r.style.setProperty(`--egg-sx`,((Math.random()*2-1)*(s+c)).toFixed(1)+`px`),r.style.setProperty(`--egg-sy`,((Math.random()*2-1)*s).toFixed(1)+`px`),r.style.setProperty(`--egg-rot`,l.toFixed(2)+`deg`),r.style.setProperty(`--egg-scl`,u.toFixed(4))},80),p=(e,t)=>setTimeout(t,e),h=E(`egg-border-flicker`);h.classList.add(`run`),p(600,()=>h.remove());let g=document.getElementById(`stageName`);p(400,()=>g&&g.classList.add(`egg-rgb`));for(let e=0;e<5;e++)p(400+Math.random()*900,()=>{let e=document.createElement(`div`);e.className=`egg-band`,e.style.top=Math.random()*90+`vh`,e.style.height=18+Math.random()*26+`px`;let t=(20+Math.random()*40)*(Math.random()<.5?-1:1);e.style.transform=`translateX(${t}px)`,document.body.appendChild(e),c=30,setTimeout(()=>{c=0,e.remove()},100)});p(1400,()=>g&&g.classList.remove(`egg-rgb`));let _=null,v=null;p(800,()=>{_=D(),v=E(`egg-scanlines`),requestAnimationFrame(()=>v.classList.add(`show`))}),p(1200,()=>{document.querySelectorAll(`#stageName .ltr`).forEach(e=>{O(e,`rotate(${((20+Math.random()*100)*(Math.random()<.5?-1:1)).toFixed(0)}deg)`,.9,Math.random()*.4)}),document.querySelectorAll(`.topbar .pill, #contact .pill`).forEach(e=>{let t=e.getBoundingClientRect();O(e,`rotate(${(Math.atan2(window.innerHeight/2-(t.top+t.height/2),window.innerWidth/2-(t.left+t.width/2))*180/Math.PI).toFixed(0)}deg) scaleX(1.7)`,1,.15+Math.random()*.35)})});let y=E(`egg-flash`);p(2600,()=>{y.style.transition=`opacity 0.12s ease-in`,y.style.opacity=`1`,setTimeout(()=>{y.style.transition=`opacity 0.25s ease-out`,y.style.opacity=`0`},130),document.getElementById(`sky`)?.classList.add(`egg-burnout`),document.querySelector(`.dotgrid`)?.classList.add(`egg-burnout`),d=!0}),p(3600,()=>{k(e=>{clearInterval(f),i.forEach(e=>e.classList.remove(`egg-shaken`)),[`--egg-sx`,`--egg-sy`,`--egg-rot`,`--egg-scl`].forEach(e=>r.style.removeProperty(e)),_?.remove(),v?.remove(),h.remove(),y.remove(),t.remove(),A(e,()=>{let e=E(`egg-blackout`);e.style.transition=`opacity 0.5s ease`,requestAnimationFrame(()=>e.style.opacity=`1`),setTimeout(M,520)})})})}function M(){let e=document.createElement(`div`);e.className=`egg-dead`,e.innerHTML=`
